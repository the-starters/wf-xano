...
| `wf-xano-lazy-details="true"` | any value (presence enables) | Optional marker on a `details-target` inside a list template. Its authored child subtree stays detached and unbound while the panel is closed, then hydrates exactly once when that card is opened. Use this for large hidden project, invoice, or audit panels so append pagination does not multiply offscreen DOM work. Later closes preserve the hydrated panel and reopening does not duplicate nested rows. |
