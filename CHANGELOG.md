...
- New `wf-xano-element="pagination-wrapper"`: marks the element wrapping the whole pagination UI
  and hides it whenever the result fits on a single page, so a one-page result no longer leaves
  inert pager chrome on the page. Respects `wf-xano-display` for the shown value and re-shows when
  a later result spans more pages.
